...
export class NewOkrComponent implements OnInit, OnChanges {
  ...
  save(): void {
    ...
    if (!this.okr.objective.trim()) {
      ...
      this.okrService.createOkr(this.okr).subscribe((okr: Okr) => {
        if (okr.parent) {
  ...
  getParent() {
    if (this.parentId) {
      this.okrService.getOkr(this.parentId).subscribe((okr: Okr) => {
        this.parent = okr;
  ...
  addToParentOnSave(okr: Okr) {
    this.okrService.addChild(okr.parent, okr._id).subscribe(() => {
      this.savedOkr.emit(new OkrNode(okr));
    });
  }
  ...
  focusObjectiveInput() {
    setTimeout(() => this.objectiveInputEl?.nativeElement.focus(), 100);
  }

  // Called when toggling the modal
  initForm() {
    this.clearForm();

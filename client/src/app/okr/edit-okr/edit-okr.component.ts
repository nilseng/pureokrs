...
export class EditOkrComponent implements OnChanges {
  ...
  @ViewChild("objectiveInput") objectiveInputEl: ElementRef;
  ...
  ngOnChanges() {
    this.focusObjectiveInput();

    this.userService
      .getUser(this.okrHierarchyNode.data.okr.userId)
      .subscribe((owner) => (this.owner = owner));
    this.parent = this.okrHierarchyNode.parent.data.okr;
    this.noObjective = false;
    this.descendants = this.okrHierarchyNode
      .descendants()
      .map((okr) => okr.data.okr._id);
    ...
    this.parents$ = this.parentSearchTerms.pipe(
      ...
      map((parents) =>
        parents.filter(
          (parent) =>
            !this.descendants.includes(parent._id) &&
            parent._id !== this.okrHierarchyNode.data.okr.parent
        )
      )
    );
  }
  ...
  save(): void {
    this.okrService
      .updateOkr(this.okrHierarchyNode.data.okr)
      .subscribe((okr: Okr) => {
        if (
          okr.parent &&
          okr.parent !== this.okrHierarchyNode.parent.data.okr._id
        ) {
  ...
  changeParent(okrNode: OkrNode) {
    ...
    this.okrHierarchyNode.parent.data.children.splice(
      this.okrHierarchyNode.parent.data.children.indexOf(
        this.okrHierarchyNode.data
      ),
      1
    );
    if (this.okrHierarchyNode.parent.depth === 0) {
      ...
      // If the old parent is not the root node, the okr needs to be removed from the parent's children array on the server also
      this.okrService
        .removeChild(this.okrHierarchyNode.parent.data.okr._id, okrNode.okr._id)
        .subscribe(() => {
          this.addChildToParent(okrNode);
        });
    }
  ...
  addChildToParent(child: OkrNode) {
    this.okrService.addChild(child.okr.parent, child.okr._id).subscribe(() => {
      this.clearForm();
  ...
  clearForm() {
    this.auth.getUserDetails().subscribe((u) => (this.owner = u));
    this.parent = undefined;
    this.noObjective = false;
  }

  focusObjectiveInput() {
    setTimeout(() => this.objectiveInputEl?.nativeElement.focus(), 1000);
  }
}
